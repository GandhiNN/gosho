...
AWS SSO login CLI that forces fresh browser sessions (InPrivate/Incognito) per login, solving the problem of cached SAML credentials when switching between environments (e.g., DEV vs PRD) under the same SSO start URL.

## Why

`aws sso login` reuses the browser session cache, so when you need different SAML credentials for DEV vs QA+PRD, the second login silently uses the wrong cached session. Gosho opens an InPrivate/Incognito window each time, ensuring you're prompted for the correct credentials.
